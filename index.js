...
function continueClicked()
{
    $('#rightOrWrong').on('click', '#nextQuestion', event =>
    {
        ...
        {
            if (tallyYo === questionArray.length)
            {
                location.href = "QuizConclusionPerfect.html";
            }
            else
            {
                location.href = "QuizConclusionNotPerfect.html?numRight=" + tallyYo + "numQuest-" + questionArray.length;
            }
        }
        else
        {
...
function answerSubmitted()
{
    $('form').on('click', '.submitButton', event =>
    {
        ...
        $("div#rightOrWrong").append(`${response}<br><br><img src="${picture}"><br><button id = "nextQuestion">Continue</button>`);   //// I INITIALLY HAD THIS AS .TEXT, BUT THEN IT WOULDN'T APPLY TAGS SUCH AS <I>
        $("#rightOrWrong").toggleClass("here");
        questionNumber++;
    });
}
...
function postQuestion()
{
    ...
    if (questionArray[questionNumber].picture !== "")
    {
    ...
    {
        $("form").append(`<input type = "radio" name = "Question" value = "${i}" required /><label for="Question"> ${questionArray[questionNumber].answers[i]}</label>`);
        console.log(questionArray[questionNumber].answers[i].length);
    }
    $("form").append(`<br><input type="submit" class="submitButton" value="Submit">`);
}  
